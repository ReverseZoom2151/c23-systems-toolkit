...
static const numeric_type types[] = {{"i8", 8, 1},   {"u8", 8, 0},   {"i16", 16, 1},
                                     {"u16", 16, 0}, {"i32", 32, 1}, {"u32", 32, 0},
                                     {"i64", 64, 1}, {"u64", 64, 0}};
...
static void print_usage(FILE *stream) {
    fprintf(
        stream,
        "usage: binary-tool encode|decode [--hex] [--group] [--explain] TYPE VALUE\n");
    fprintf(stream, "TYPE: i8, u8, i16, u16, i32, u32, i64, or u64\n");
}

static int parse_decimal(const numeric_type *type, const char *text,
                         uint64_t *encoded) {
    char *end = NULL;
    errno = 0;
    if (type->is_signed) {
        intmax_t value = strtoimax(text, &end, 10);
        if (errno != 0 || *text == '\0' || *end != '\0' || value < INT64_MIN ||
            value > INT64_MAX)
            return 0;
        if (type->bits < 64 && (value < -(INT64_C(1) << (type->bits - 1)) ||
                                value >= (INT64_C(1) << (type->bits - 1))))
            return 0;
        *encoded = (uint64_t)value & type_mask(type);
        ...
        if (errno != 0 || *text == '\0' || *end != '\0' || value > UINT64_MAX ||
            ((uint64_t)value & ~type_mask(type)) != 0)
            return 0;
...
static void print_explanation(const numeric_type *type, uint64_t encoded) {
    ...
    (void)binary_encode_hex_unsigned(encoded, type->bits, hexadecimal,
                                     sizeof(hexadecimal));
    printf("type: %s\nwidth: %u bits\nsigned: %s\nbinary: %s\nhexadecimal: "
           "0x%s\nunsigned interpretation: %" PRIu64 "\n",
           type->name, type->bits, type->is_signed ? "yes" : "no", grouped, hexadecimal,
           encoded);
    if (type->is_signed) {
...
int main(int argc, char **argv) {
    ...
    while (argument < argc && strncmp(argv[argument], "--", 2) == 0) {
        if (strcmp(argv[argument], "--hex") == 0)
            hexadecimal = 1;
        else if (strcmp(argv[argument], "--group") == 0)
            group = 1;
        else if (strcmp(argv[argument], "--explain") == 0)
            explain = 1;
        else {
            print_usage(stderr);
            return EXIT_FAILURE;
    ...
    if (argc - argument != 2 || (hexadecimal && group)) {
        if (hexadecimal && group)
            fprintf(stderr, "--group cannot be used with --hex\n");
        print_usage(stderr);
        return EXIT_FAILURE;
    }
    const numeric_type *type = find_type(argv[argument]);
    if (type == NULL) {
        fprintf(stderr, "TYPE must be i8, u8, i16, u16, i32, u32, i64, or u64\n");
        return EXIT_FAILURE;
    }
    const char *text = argv[argument + 1];
    if (strcmp(argv[1], "encode") == 0) {
        uint64_t encoded;
        if (!parse_decimal(type, text, &encoded)) {
            fprintf(stderr, "value does not fit the requested type\n");
            return EXIT_FAILURE;
        }
        if (explain)
            print_explanation(type, encoded);
        else if (hexadecimal) {
            char output[17];
            (void)binary_encode_hex_unsigned(encoded, type->bits, output,
                                             sizeof(output));
            puts(output);
        } else {
            char binary[65];
            ...
            (void)binary_encode_unsigned(encoded, type->bits, binary, sizeof(binary));
            if (group)
                (void)binary_format_grouped(binary, 4, output, sizeof(output));
            else
        ...
        if (!hexadecimal) {
            if (!strip_group_separators(text, normalized)) {
                fprintf(stderr, "invalid grouped binary value\n");
                return EXIT_FAILURE;
            }
            input = normalized;
        }
        ...
        binary_status status =
            hexadecimal ? binary_decode_hex_unsigned(input, type->bits, &encoded)
                        : binary_decode_unsigned(input, type->bits, &encoded);
        if (status != BINARY_OK) {
            fprintf(stderr, "%s\n", binary_status_message(status));
            return EXIT_FAILURE;
        }
        if (explain)
            print_explanation(type, encoded);
